...
class Target:
    def __init__(self, pointRange: List, pointDisplayDir: int = 3, totalSize: float = 1, dpi: int = 100):
    ...
    def targetHit(self, point: int, dir: Optional[int] = None):
        ...
        if point in self.pointRange:
            ...
            for i in range(len(self.pointRange)):  # Loop through every ring
                ...
                # Set array of 12 elements, all with the ring value for even segentation
                ...
                if self.pointRange[i] == point:
                    if dir is None:  # If no direction selected
                ...
                self.ax.pie(
                    ...
                    radius=size * (i + 1),
                    colors=color,
                    autopct=lambda txt: self.func(val),
        ...
        # Create "Träff" target display
            ...
            self.ax.pie(
                ...
                radius=totalSize,
                colors=color,
                wedgeprops=dict(width=totalSize, edgecolor="black"),
                startangle=45,
            )
        ...
        # Create "Miss / Bom" target display
            ...
            self.ax.pie(
                [1],
                radius=totalSize,
                colors=[self.getMissColor()],
                wedgeprops=dict(width=totalSize, edgecolor="black"),
                startangle=45,
                autopct=lambda txt: "Miss",
                pctdistance=0,
                textprops=dict(color="white", size="xx-large"),
            )
